...
file_descriptor_t output_file = { .ref_count = 0, .lock = MUTEX_UNLOCKED };
...
// Log a message to /var/log/syslog
void log_output(int8_t* format, ...) {
	if (output_file.ref_count == 0) {
		if (!fopen("var/log/output.log", FILE_MODE_CREATE | FILE_MODE_APPEND, &output_file)) {
			// Create the directories var and var/log
			fmkdir("var");
			fmkdir("var/log");
			if (!fopen("var/log/output.log", FILE_MODE_CREATE | FILE_MODE_APPEND, &output_file)) {
		...
		output_file.ref_count = 1;
	}
	
	char str[1024];
	int len = sget_current_date(str);
	fwrite(str, 1, len, &output_file);
	fwrite(" - ", 1, 3, &output_file);
